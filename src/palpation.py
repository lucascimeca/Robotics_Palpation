...
def run_learning(time_interval=1.,
    ...
    if testing:
        ...
        bayesian_brain = BayesianBrain(
            ...
            experiment_number='abdominal_phantom',
            verbose=False,
    ...
    palpation = Palpation(interval=time_interval)  # start palpation on main thread
    doctor = Doctor(
...
class Doctor(BodyObserver):

    def __init__(self, name="Robo-doctor", robot_ip="169.254.71.113", learning=False, environment=None, actions=None,
                 sample_number=3, sensing_resolution=10, palpation_duration=3, downtime_between_experiments=8,
        ...
        self.palpation_joint_start = np.deg2rad([-4.70, -63.81, 113.26, -139.80, -91.88, -34.15]) # training
        # self.palpation_joint_start = np.deg2rad([83.73, -109.20, 121.96, 258.39, -89.19, -14.06]) # testing
        # self.palpation_joint_start = np.deg2rad([-4.80, -74.73, 107.30, -123.00, -85.79, -34.27]) # demo
        self.robot = UR5(robot_ip=robot_ip)
        self.robot.joint_go_to(self.palpation_joint_start, acc=.5, vel=1)
        print('Waiting for robot to reach start position...')
        while not self.robot.reached_point(point=self.palpation_joint_start, mode='joint'): pass
    ...
    def _update_doctor_state(self):
        ...
        if np.any(self.skin_snapshot[-1, :] == self.skin_snapshot[0, :]):
        ...
        if not self.robot_stop:
            if not self.force_limit_procedure or self.skin_error_count <= 4 or self.robot_connection_lost:
                ...
                if self.reached_palpation_positon is None:
                    ...
                    # self.robot.set_tcp(self.descent_start_pose)
    ...
    def release(self):
        ...
        if self.verbose:
            print("Going home!")
        self.robot.joint_go_to(self.palpation_joint_start, acc=.5, vel=1)
        while not self.robot.reached_point(point=self.palpation_joint_start, mode='joint'):
            pass
        print("All done.")
        # ----- disconnect -----
...
class Brain(object):
    ...
    def __init__(self, body=None, learning=False,  environment=None, actions=None, priors=None, sensing_resolution=10,
                 dimensionality_reduction=2, samples_per_pdf=3, training_ratio=.8, test_mode=False, verbose=False):
            ...
            if act not in reduced_belief_data.keys():
                ...
                for obj in sorted(self.belief_data[act].keys()):
                    ...
                    if mat_all_objs is None:
                        mat_all_objs = self.belief_data[act][obj]
                    else:
                        mat_all_objs = np.append(arr=mat_all_objs,
                                                 values=self.belief_data[act][obj],
                                                 axis=0)
    ...
    def _execute_motion(self, moving, step):
        ...
        if not self.body.end_palpate:
            if not self.body.touched:
                ...
                if not moving:
                    self.body.robot.set_speed(vel=-.01, acc=.05, t=10)  # go down
                else:
                    if self.verbose:
                    ...
                    if np.count_nonzero(np.array(skin_diff > 30)) >= 3 and self.body.state['actual_TCP_pose'][2] < 0.09:
                        ...
                        if self.body.detected_contact_number >= 3 \
            ...
            if not moving:
                ...
                if self.body.descent_start_pose[2] - self.body.state['actual_TCP_pose'][2] <= 0.0001:
                    ...
                    if self.learning:
                        ...
                        if self.current_motion[0] not in self.belief_data[self.body.current_motion[2][0]]:
                            self.belief_data[self.body.current_motion[2][0]][self.current_motion[0]] = \
                                (self.skin_buffer - self.skin_buffer[0])[1:].reshape(1, -1)
                        else:
                            self.belief_data[self.body.current_motion[2][0]][self.current_motion[0]] = np.append(
                                arr=self.belief_data[self.body.current_motion[2][0]][self.current_motion[0]],
                        ...
                        for obj in self.reduced_belief_data[self.body.current_motion[2][0]].keys():
                            if X_train is None:
                                ...
                                X_train = np.append(arr=X_train,
                                                    ...
                                                    axis=0)
                            y_train += [self.objects.index(obj)] * \
                        ...
                        plot_robot_inference1d(
                            ...
                            X_train=X_train,
                            y_train=y_train,
                            action_number=self.body.current_motion[2][0],
                            show=True
                        )
