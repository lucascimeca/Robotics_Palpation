...
class DoctorViewer(SincDataViewer):
    ...
    def show_frame(self, step, frame_rate=1):
        ...
        for ihb in range(self.ihb_num):
            for module in range(self.module_num):
                heatmap, skin_array = skin_heatmap(self.detector.skin_snapshot, max=20000)
        ...
        axes_rx.title.set_text('Rx Profile')
        axes_rx.plot(np.arange(40)*(1/frame_rate), self.action_profile_buffer[:40, 3], 'r--', label="control velocity")
        ...
        axes_ry.title.set_text('Ry Profile')
        axes_ry.plot(np.arange(40)*(1/frame_rate), self.action_profile_buffer[:40, 4], 'r--', label="control velocity")
...
def show_svm(pdfs=None, act=None, reduced_train_data=None, reduced_test_data=None, folder="", show=True, save=False,
...
def plot_robot_inference2d(X_train=None, y_train=None, X_test=None, y_test=None, predictions=None, clf=None, pdfs=None,
                           windows_name="inference", folder="", show=~False, save=False):
    if windows_name is not None:
        windows_name = windows_name + "_inference"
    ...
    if clf is not None:
        ...
        plot_bayesian_contours(ax, pdfs, xx, yy, cmap=plt.cm.coolwarm, alpha=0.8)
...
def plot_robot_inference1d(pdfs=None, X_train=None, y_train=None, X_test=None, y_test=None, predictions=None, action_number=None,
                           windows_name="inference", folder="", show=False, save=False):
        ...
        for i, obj in enumerate(sorted(pdfs.keys())):
            mu, sig = pdfs[obj]
            ax_gauss.plot(xs, gaussian(xs, mu, sig), label=labeltag_dict[obj], c=colors[i], linewidth=4, alpha=.5)
