...
class BayesianBrain(object):
    ...
    def __init__(self, environment=None, actions=None, priors=None, sensing_resolution=10,
                 dimensionality_reduction=2, samples_per_pdf=3, training_ratio=.8, experiment_name=False,
                 data_folder='./../data/', verbose=False):

        # REMOVE THE -1 IF YOU'RE NOT SUBTRACTING THE STARTING CONDITION
        if sensing_resolution < 2:
            raise ValueError("The value of 'sensing_resolution' must be >=2!")
        ...
        base_name = ""
        experiment_folders = list(sorted([file for file in os.listdir(data_folder)]))
        ...
        # TO REMOVE
        if experiment_name:
            self.data_folder = data_folder + experiment_name + "/"
        else:
    ...
    def run_experiment(self, bayesian=True, initial_sample_number=2, test_sample_number=6,
                       previous_accuracies=None, maximum_iterations=False, show=True, save=False):
        ...
        if bayesian:
            while new_belief_data is not None and test_data is not None:
                ...
                identifiers = [act_idx for act_idx, _, _ in self.actions]
                idx = np.where(np.array(identifiers) == np.array(actions_to_explore[-1]))[0]

                if actions_to_explore[-1] not in running_accuracies.keys():
                    running_accuracies[actions_to_explore[-1]] = [(self.action_counts[actions_to_explore[-1]], accuracy)]
                    running_ba[actions_to_explore[-1]] = [(self.action_counts[actions_to_explore[-1]], unbiased_action_benefits[idx])]
                else:
                    running_accuracies[actions_to_explore[-1]] += [(self.action_counts[actions_to_explore[-1]], accuracy)]
                    running_ba[actions_to_explore[-1]] += [(self.action_counts[actions_to_explore[-1]], unbiased_action_benefits[idx])]
                ...
                best_action_benefits += [action_benefits[0]]
                explored_accuracies += [accuracy]
                iterations_b_coeffs += [unbiased_action_benefits]
        ...
        if show or save:
            ...
            show_svm(pdfs=self.pdfs,
                     act=actions_to_explore[-1],
                     reduced_train_data=best_reduced_test_data,
                     reduced_test_data=best_reduced_test_data,
                     folder=self.results_folder,
                     show=show,
                     save=save)
            plt.close("all")
        ...
        for obj_keys in acc_test_dict:
            temp_accs = []
            if np.all([x in self.objects for x in obj_keys]):
                for act in sorted(self.pdfs.keys()):
                    accuracy = self.test_SVM(
                        act=act,
                        reduced_belief_data=reduced_belief_data,
                    ...
                    temp_accs += [accuracy]
                print("{} highest accuracy: {}, avg accuracy: {}".format(obj_keys, np.max(temp_accs), np.average(temp_accs)))
        ...
        data['running_accuracies'] = running_accuracies
        data['running_ba'] = running_ba
    ...
    def _load_initial_data(self, initial_sample_number=3, test_sample_number=3):
        # ------------ Load Data from files ----------

        if initial_sample_number == 0:
            initial_sample_number = self.samples_per_pdf - test_sample_number

        if initial_sample_number + test_sample_number > self.samples_per_pdf:
            raise ValueError("The number of initial samples and test samples must be smaller than all the "
                             "collected samples, or it will be impossible to iterate.")

        self.test_samples_per_pdf = test_sample_number
        ...
        for act, _, _, in self.actions:
            ...
            for obj in self.objects:
                ...
                if initial_sample_number + self.test_samples_per_pdf > len(obj_act_indexes):
            ...
            if act not in self.missing_actions:
                if act not in self.original_belief_data.keys() or act not in self.original_test_data.keys():
                    ...
                    self.original_test_data[act] = np.zeros((
                        self.number_of_objects * self.test_samples_per_pdf,
                        self.sensing_resolution * np.prod(list(self.loaded_skin_shape))
                    ))
                    ...
                    if obj not in self.missing_objects[act]:
                        ...
                        for sample_no in sample_belief_idxs[act][obj] + sample_test_idxs[act][obj]:
                            ...
                            if skin_filename.split("/")[-1] in self.skin_files:
                                ...
                                if sample_no in sample_belief_idxs[act][obj]:
                                    self.original_belief_data[act][self.belief_idx[act], :] = skin_data[1:self.sensing_resolution+1].flatten()
                                    self.belief_idx[act] += 1
                                else:
                                    self.original_test_data[act][self.test_idx[act], :] = skin_data[1:self.sensing_resolution+1].flatten()
                                    self.test_idx[act] += 1
                            else:
        ...
        for obj in self.objects:
            ...
            if skin_filename.split("/")[-1] in self.skin_files:
                ...
                self.original_belief_data[act] = np.append(arr=self.original_belief_data[act],
                                                      values=skin_data[1:self.sensing_resolution+1].reshape(1, -1),
                                                      axis=0)
                self.belief_idx[act] += 1
            else:
                return None
    ...
    def _fit_pdfs(self, belief_data, test_data, act=None, reduced_belief_data=None, reduced_test_data=None,
            ...
            if act not in self.missing_actions:
                ...
                for obj in self.objects:
                    if obj not in self.missing_objects[act]:
                        ...
                        # TEST DATA - only one (first) set of indexes added at the beginning, robot does not change testing data
                        mat_obj_test_copy[obj] = test_data[act][
                ...
                for obj in self.objects:
                    ...
                    mat_obj_test_copy[obj] = (mat_obj_test_copy[obj] - self.normalize_min[act]) / (
                            self.normalize_max[act] - self.normalize_min[act])
                    reduced_test_mat = self.pcas[act].transform(mat_obj_test_copy[obj])
                    ...
                    reduced_test_data[act][obj] = reduced_test_mat
    ...
    def test_SVM(self, act=0, reduced_belief_data=None, reduced_test_data=None, obj_keys=None, show=False):
        ...
        test_data = np.zeros((self.test_samples_per_pdf * len(obj_keys), self.dimensionality_reduction))
        training_labels = []
        test_labels = []
        ...
        for j, obj in enumerate(obj_keys):
            if obj not in self.missing_objects[act]:
                ...
                # TEST DATA
                test_data[j * self.test_samples_per_pdf:j * self.test_samples_per_pdf + self.test_samples_per_pdf, :] = \
                    reduced_test_data[act][obj]
                test_labels += [label_dict[obj]] * self.test_samples_per_pdf
...
if __name__ == "__main__":
    ...
    brain = BayesianBrain(
        ...
        experiment_name='exploration_training_phantom_1',
        verbose=False,
    ...
    print("systematic")
    data = brain.run_experiment(bayesian=False,
                                maximum_iterations=200,
                                initial_sample_number=2,
    ...
    print("bayesian")
    brain.run_experiment(bayesian=True,
                         maximum_iterations=200,
                         previous_accuracies=data['best_accuracies'],
                         initial_sample_number=2,
                         test_sample_number=9,
                         show=True,
                         save=False)
    ...
    # data = brain.run_experiment(bayesian=True, show=True)

    # print("\n best bayesian action: {}".format(best_explored_action))
